...
import { AGENT_MODELS, AGENT_EFFORTS, DEFAULT_AGENT_MODEL, CODEX_MODELS, GROK_MODELS, AGENT_PROVIDERS, providerLabel } from "../../convex/lib/agentModel";

type Draft = Pick<Doc<"agents">, "provider" | "name" | "description" | "model" | "effort" | "guidance" | "skillIds"> & { agentId?: Id<"agents"> };
...
export function AgentsPage() {
  ...
  const visible = agents?.filter(a => `${a.name} ${a.description} ${a.model} ${providerLabel(a.provider)}`.toLowerCase().includes(search.toLowerCase()));
  return <>
    ...
    {agents === undefined ? <p className="muted">Loading Agents…</p> : !agents.length ? <section className="card agents-empty"><div className="agent-avatar">A</div><h2>Build your first specialist</h2><p className="muted">Create a planner, builder, or reviewer with the model and Skills its work needs.</p><button onClick={() => edit(blank())}>Create Agent</button></section> : !visible?.length ? <p className="muted">No Agents match your search.</p> : <div className="agents-grid">{visible.map(a => <article className="card agent-card" key={a._id}>
      ...
      <div className="row"><span className="badge">{providerLabel(a.provider)}</span><span className="badge">{a.model}</span><span className="badge">{a.effort} effort</span></div><p className="muted">{a.skillIds.length} Skills</p>
      <div className="row"><button className="ghost" onClick={() => edit({ agentId: a._id, name: a.name, description: a.description, provider: a.provider, model: a.model, effort: a.effort, guidance: a.guidance, skillIds: a.skillIds })}>Edit Agent</button><button className="ghost" onClick={() => edit({ name: `${a.name} copy`, description: a.description, provider: a.provider, model: a.model, effort: a.effort, guidance: a.guidance, skillIds: a.skillIds })}>Duplicate</button></div>
    </article>)}</div>}
    {draft && <dialog ref={dialog} className="agent-dialog" aria-labelledby="agent-editor-title" onCancel={e => { e.preventDefault(); if (!busy) setDraft(null); }}><section className="card agent-editor"><form className="stack" onSubmit={e => { e.preventDefault(); void persist(); }}>
      ...
      <label>Provider<select value={draft.provider ?? ""} onChange={e => {
        const provider = (e.target.value || undefined) as Draft["provider"];
        setDraft({ ...draft, provider, model: provider === "codex" ? CODEX_MODELS[0] : provider === "grok" ? GROK_MODELS[0] : provider === "cursor" ? DEFAULT_AGENT_MODEL : draft.model });
      }}><option value="">Worker default</option>{AGENT_PROVIDERS.map(p => <option key={p} value={p}>{providerLabel(p)}</option>)}</select></label>
      <div className="agent-settings"><label>Model<input required list="agent-models" value={draft.model} onChange={e => setDraft({ ...draft, model: e.target.value })} /><datalist id="agent-models">{(draft.provider === "codex" ? CODEX_MODELS : draft.provider === "grok" ? GROK_MODELS : AGENT_MODELS).map(m => <option key={m} value={m} />)}</datalist></label><label>Effort<select value={draft.effort} onChange={e => setDraft({ ...draft, effort: e.target.value as Draft["effort"] })}>{AGENT_EFFORTS.map(e => <option key={e} value={e}>{e}</option>)}</select></label></div>
      <p className="muted">{draft.provider === "codex" ? "Codex runs locally on this machine. Choose a suggested model or enter a model ID available to your Codex account. Effort support depends on the model." : draft.provider === "grok" ? "Grok Build runs locally and can use the Grok subscription signed in on this machine, or an xAI API key." : "Choose a configured model or enter a model ID supported on this machine."}</p>
