  ...
  return <>
    ...
    <section className="settings-section"><div className="section-heading"><div><h2>Connected Projects</h2><p>Use the sidebar switcher to choose your active Project.</p></div></div>
      <div className="settings-project-list">
        {projects?.map((project) => <div className="settings-project" key={project._id}><div><strong>{project.name}</strong><span>{project.githubRepo || project.localPath}</span></div><div className="row"><Link className="text-button" to={`/projects/${project._id}`}>Edit</Link><button className="danger-button" type="button" onClick={() => void removeProject(project._id, project.name)}>Remove</button></div></div>)}
      </div>
    </section>
    <section className="settings-section add-project"><div className="section-heading"><div><h2>Add Project</h2><p>Register the repository a Job should run against.</p></div></div>
      <form className="project-form" onSubmit={(event) => void addProject(event)}>
        ...
        <label>GitHub repo<input value={githubRepo} onChange={(event) => setGithubRepo(event.target.value)} placeholder="owner/repo" /></label>
