...
export function EnvironmentManager({ scope }: { scope: string }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (name === "FACTORY_PROVIDER" && !["cursor", "openai"].includes(value)) throw new Error("FACTORY_PROVIDER must be cursor or openai.");
  ...
  return <div className="environment-manager"><h3>{scope === "server" ? "Worker environment" : "Project environment"}</h3><p className="muted">Values are encrypted for this worker and hidden after saving. No Project .env file is written.</p>
    {scope === "server" && <p className="muted">Choose cursor or openai for FACTORY_PROVIDER. Set its API key here. Saved settings override the worker’s startup environment.</p>}
