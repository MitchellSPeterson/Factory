...
export function WorkerPairing() {
  ...
  return <section id="machine" className="settings-block"><div className="section-heading"><div><h2>This machine</h2><p>This machine clones repositories and runs your Jobs.</p></div></div>
    {live === undefined ? <p className="muted">Checking this machine…</p> : live ? <div className="settings-option"><div><strong>{live.name} · {online ? "Online" : "Offline"}</strong><span>Repositories: {live.projectsRoot}</span></div></div> : <p className="muted">Start the worker on this machine. It registers itself with this Factory.</p>}
  </section>;
}
...
export function WorkerSettings() {
  ...
  return <>
    <WorkerPairing />
    {live ? <ProviderCards /> : null}
  </>;
}

type ProviderCardProps = { name: string; description: string; variable?: typeof serverVariableNames[number]; placeholder?: string; status?: string; children?: ReactNode };
...
function GrokProviderCard() {
  const live = useQuery(api.servers.local);
  const catalog = live?.grokCatalog;
  const status = !catalog
    ...
    : !catalog.installed
      ...
      : catalog.authenticated === false
        ? "Sign in"
        : "Ready";
  const models = (catalog?.models ?? []).map((model) => model.slug).join(", ");
  return (
    <ProviderCard
      name="Grok Build"
      description="Sessions speak Grok's ACP on this machine. Stage Runs still use the Grok CLI."
    ...
    >
      <p className="muted">
        {catalog?.message
          ?? (models !== "" ? `Models from this machine: ${models}.` : "Install with `npm install -g @xai-official/grok`, then run `grok login`.")}
      </p>
    </ProviderCard>
  );
}

function ProviderCard({ name, description, variable, placeholder, status, children }: ProviderCardProps) {
  ...
  const badge = status ?? (configured ? "Key saved" : name === "Cursor" ? "Setup required" : "CLI login supported");
  return <article className="provider-card"><div className="provider-card-heading"><div><h3>{name}</h3><p>{description}</p></div><span className={`provider-status ${configured || status === "Ready" ? "configured" : ""}`}>{badge}</span></div>{children}<form className="provider-form" onSubmit={submit}><input type="password" autoComplete="off" aria-label={placeholder} placeholder={configured ? `${placeholder} (replace)` : placeholder} value={value} onChange={event => setValue(event.target.value)} /><button disabled={busy || !value}>{busy ? "Saving…" : configured ? "Replace key" : "Save key"}</button>{configured && <button type="button" className="ghost" disabled={busy} onClick={async () => { setBusy(true); try { await remove({ accessKey, scope: "server", name: variable! }); setMessage("Saved key removed."); } finally { setBusy(false); } }}>Remove</button>}</form>{message && <p className="muted" role="status">{message}</p>}</article>;
}
export function EnvironmentManager({ scope }: { scope: string }) {
  ...
  if (!server) return <p className="muted">Start the worker on this machine to manage environment variables.</p>;
