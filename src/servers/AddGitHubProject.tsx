...
export function AddGitHubProject() {
  ...
  return <section className="settings-section"><div className="section-heading"><div><h2>Add from GitHub</h2><p>Choose a repository. Factory clones it on your paired worker automatically.</p></div></div>
    {!connection ? <Link className="text-button" to="/github">Connect GitHub first →</Link> : !server ? <p className="muted">Pair a worker in Settings to choose where the repository is cloned.</p> : <form className="project-form" onSubmit={async event => {
      ...
      <p className="muted">Clone on {server.name}. Your GitHub connection needs Contents read permission. Its token is encrypted for this worker and removed from the import after cloning.</p>
...
export function RetryClone({ projectId }: { projectId: Id<"projects"> }) {
  ...
  return <><button className="ghost" disabled={!connection || !server || busy} onClick={async () => { if (!connection || !server) return; setBusy(true); setError(""); try { await retry({ projectId, accessKey, sealedToken: await sealSecret(server.publicKey, connection.token) }); } catch { setError("Unable to retry. Check the worker pairing and GitHub connection."); } finally { setBusy(false); } }}>Retry clone</button>{(!connection || !server) && <p className="muted">Connect GitHub and pair this worker in Settings to retry.</p>}{error && <p className="error" role="alert">{error}</p>}</>;
}
