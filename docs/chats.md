...
Choose a conversation from the searchable list, or start a new chat and choose a Project. Project scope filters the list. The composer supports text, image attachments, slash commands, repo Skills, and a model picker with per-provider options: reasoning effort, runtime access mode, and Codex service tier. Type `/` to pick `/model`, `/compact` in an existing conversation, or a Skill from the Project (`.agents/skills`, `.claude/skills`, `.codex/skills`, `.grok/skills`, `skills`) and this machine's installed Skills (`~/.agents/skills`, `~/.claude/skills`, `~/.codex/skills`, `~/.cursor/skills`, `~/.grok/skills`, and Grok's bundled catalog). Chosen Skills ride with the message as `skill:slug` mentions so the agent can load them from disk. `/compact` asks the agent to summarize the thread so later turns carry less context. Messages render Markdown and images; permission prompts let the operator answer the agent. A running conversation shows its state and a Stop agent action. The Latest message action returns to the bottom after scrolling up.
