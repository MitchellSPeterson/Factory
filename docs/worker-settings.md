...
Managed Projects stay on this machine. Jobs cannot start until cloning finishes. GitHub credentials used for importing are temporary; later git push/pull operations still use this machine's existing git authentication.

## Environment manager

**Settings → Worker → Worker environment** supports:

- `FACTORY_PROVIDER`: `cursor`, `codex`, `grok`, or `openai`; used when the Agent has no explicit provider.
- `CURSOR_API_KEY`.
...
Use a **local** Job: the Codex SDK runs against the Project checkout on this machine. Cloud Jobs are currently supported only by Cursor. The worker includes `@openai/codex-sdk` and its CLI runtime after `bun install`. Authenticate Codex on this machine, or save `CODEX_API_KEY` in Worker environment (`OPENAI_API_KEY` is a fallback). No Cursor key is required for a Codex Agent. `CODEX_BASE_URL` is independent of the OpenAI-compatible runner’s `OPENAI_BASE_URL`.

## Grok Build

Choose **Grok Build** for an Agent to run the official Grok CLI locally against the Project checkout. Install it on this machine with `npm install -g @xai-official/grok`, then run `grok login` once to use this machine's Grok, SuperGrok, or X Premium subscription. Factory reuses the CLI's cached, refreshable login; an xAI API key is not required. `XAI_API_KEY` is supported as an alternative for API-billed use, and `GROK_PATH` can point to a non-standard Grok executable.

Grok Build receives the Stage prompt and selected Skills, streams its text and token usage into the Run, and uses Factory's lifecycle bridge for Asks, artifacts, and `finish_stage`. Grok Runs are local-only. The worker runs Grok headlessly with workspace tools enabled and non-interactive approvals, so configure the worker account and Project checkout with the permissions you intend it to have.
