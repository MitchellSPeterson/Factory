export const serverVariableNames = ["FACTORY_PROVIDER", "CURSOR_API_KEY", "OPENAI_API_KEY", "OPENAI_BASE_URL", "OPENAI_MODEL"] as const;
export function validateVariableName(name: string, server: boolean) {
  ...
  } else if (/^(FACTORY_|CONVEX_|CURSOR_|OPENAI_|GIT_|BUN_|NODE_|LD_|DYLD_)/.test(name) || ["PATH", "HOME", "SHELL", "ENV", "BASH_ENV", "ZDOTDIR", "NODE_OPTIONS"].includes(name)) {
